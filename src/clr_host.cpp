...
typedef int (CORECLR_DELEGATE_CALLTYPE *load_fn)(const char*, const char*, const char*, const char*);

init_fn s_init = nullptr;
load_fn s_load = nullptr;

void* resolve(load_assembly_and_get_function_pointer_fn loadfn, const char* asmPath, const char* method) {
    void* fn = nullptr;
    int rc = loadfn(asmPath, "Petrichor.ModHost.Host, Petrichor.ModHost",
...
bool clr_boot(const PetrichorHost* host) {
    ...
    std::string cfg     = base + "Petrichor.ModHost.runtimeconfig.json";
    std::string asmPath = base + "Petrichor.ModHost.dll";
    ...
    s_init = (init_fn)resolve(loadfn, asmPath.c_str(), "ModHost_Init");
    s_load = (load_fn)resolve(loadfn, asmPath.c_str(), "ModHost_LoadMod");
...
bool clr_loadMod(const char* dir, const char* id, const char* type, const char* entry) {
    ...
    return s_load(dir, id, type, entry ? entry : "") == 0;
}
